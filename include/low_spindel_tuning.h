...
void low_spindel_tuning(){  
      ...
      if (DBR_EE_write.fell()) {
        EEPROM.put(LOW_SPINDEL_ADDRES,lowSpinCounter + correction);
        disp.clear();
        disp.print("gone");
        disp.update();
        delay(1000);
      ...
      if (DBR_on_start_pos.fell() && DBR_low_zero_sens.read() == HIGH ){  
        ...
        while (DBR_low_zero_sens.read() == HIGH) {
          ...
          digitalWrite(LOW_SPINDEL_PULSE_OUT, HIGH);
          delayMks(50);
          digitalWrite(LOW_SPINDEL_PULSE_OUT, LOW);
          delay(1);
          i++;
        }
      ...
      //---------------------------------кнопка назад
        ...
        while (DDBR_rew_button.read() == LOW){
          ...
          DDBR_rew_button.update();
          DBR_low_zero_sens.update();
          digitalWrite(LOW_SPINDEL_DIR_OUT, HIGH);
          delayMks(10);
          digitalWrite(LOW_SPINDEL_PULSE_OUT, HIGH);
          delayMks(50);
          if (DBR_low_zero_sens.read() == HIGH) lowSpinCounter++;
          digitalWrite(LOW_SPINDEL_PULSE_OUT, LOW);
          digitalWrite(LOW_SPINDEL_DIR_OUT, LOW);
        }
      }
}