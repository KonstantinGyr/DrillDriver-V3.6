...
void top_spindel_tuning(){
      ...
      if (DBR_EE_write.fell()) {
        EEPROM.put(TOP_SPINDEL_ADDRES,topSpinCounter + correction);
        disp.clear();
        disp.print("gone");
        disp.update();
        delay(2000);
      ...
      if (DBR_on_start_pos.fell() && DBR_top_zero_sens.read() == HIGH ){ 
        ...
        while (DBR_top_zero_sens.read() == HIGH) {
          ...
          digitalWrite(TOP_SPINDEL_PULSE_OUT, HIGH);
          delayMks(50);
          digitalWrite(TOP_SPINDEL_PULSE_OUT, LOW);
          delay(1);
        }
      ...
      //-----------------------------кнопка назад
        ...
        while (DDBR_rew_button.read() == LOW){
          ...
          DDBR_rew_button.update();
          DBR_top_zero_sens.update();
          digitalWrite(TOP_SPINDEL_DIR_OUT, HIGH);
          delayMks(10);
          digitalWrite(TOP_SPINDEL_PULSE_OUT, HIGH);
          delayMks(50);
          ...
          digitalWrite(TOP_SPINDEL_PULSE_OUT, LOW);
          //delay(1);
          digitalWrite(TOP_SPINDEL_DIR_OUT, LOW);
        }
      }
}